...
describe('[GET] /enderecos/:id', () => {
    ...
    it('deve retornar 200 e um endereço', async () => {
        const endereco = await enderecoFactory({
            clienteId: cliente.id
        })

        const response = await request(app)
            .get(`/enderecos/${endereco.id.toString()}`)
    ...
    it('deve retornar 400 quando o endereço não existe', async () => {
        ...
        const response = await request(app)
            .get(`/enderecos/${idEnderecoInexistente}`)
            .auth(token, { type: 'bearer' })
            .send()
    ...
    it('deve retornar 403 quando o usuário for um cliente e o endereço não for seu', async () => {
